# app.py
# DFS Matrix — GTO Scorecard (Beta)
# Upload your weekly files, merge, and compute PO%, GTO%, projections, diagnostics

from __future__ import annotations
import io, re, unicodedata, numpy as np, pandas as pd, streamlit as st
...
def load_dk_pool(file):
    dk = pd.read_csv(file)
    # Expect columns: Name, ID, Salary, Game Info
...
def load_rg_proj(file):
    ...
    return rg[["Player","RG_fpts","RG_proj_own","RG_ceil","RG_floor","RG_salary"]]
...
def load_dg_course_fit_vertical(file):
   ...
   df = pd.read_excel(file, sheet_name=0, header=None, engine="openpyxl")
    ...
    for raw in s:
        val = raw.strip()
        if not val: continue
        # Player header heuristic
        if ("," in val) or (re.search(r"\s", val) and not re.search(r"[_/\\]", val)):
            if current_player and record: rows.append(record)
            current_player = normalize_name(val)
            record = {"Player": current_player, "SG_Baseline": np.nan, "CH_Adj": np.nan, "CF_Adj": np.nan}
            continue
        key = val.lower().replace(" ", "").replace("\u00a0","")
        if key.startswith("baseline"):
            # next numeric will fill SG_Baseline
            pass
        elif key.startswith("ch_adj"):
            pass
        elif key.startswith("cf_adj"):
            pass
        elif re.match(r"^-?\d+(\.\d+)?$", val):
            for fld in ["SG_Baseline","CH_Adj","CF_Adj"]:
                if fld in record and pd.isna(record[fld]):
                    record[fld] = float(val); break
        else:
            # ignore tee_time / decay etc.
            pass
    if current_player and record: rows.append(record)

    out = pd.DataFrame(rows)
    if "Player" in out: out["Player"] = out["Player"].map(normalize_name)
    return out
...
def load_dg_sg_history_vertical(file):
    ...
    for raw in s:
        ...
        if ("," in val) or (re.search(r"\s", val) and not re.search(r"[_/\\]", val)):
            if current_player and record: rows.append(record)
            current_player = normalize_name(val)
            record = {"Player": current_player, "SG_Putt": np.nan, "SG_ARG": np.nan, "SG_APP": np.nan, "SG_OTT": np.nan}
            continue
        if val.lower().startswith("putt"): pass
        elif val.lower().startswith("arg"): pass
        elif val.lower().startswith("app"): pass
        elif val.lower().startswith("ott"): pass
        elif re.match(r"^-?\d+(\.\d+)?$", val):
            for fld in ["SG_Putt","SG_ARG","SG_APP","SG_OTT"]:
                if pd.isna(record.get(fld, np.nan)):
                    record[fld] = float(val); break
        else:
            pass
    if current_player and record: rows.append(record)
    ...
    out["SG_Recent"] = out["SG_Long"]  # placeholder; refine later if you split windows
...
def bubble_tout(df):
    proj = _safe_minmax(df.get("RG_fpts", 0))  # use RG mean as "industry"
...
def bubble_advanced(df):
    # Use SG_Baseline/CH_Adj/CF_Adj if available; fall back to SG_Long/SG_Recent
    comp = _safe_minmax((df.get("CompField_Adjust", 0) + 1) / 2.0)
    fit  = _safe_minmax((df.get("Course_Fit", 0) + 1) / 2.0)
    sgl  = _safe_minmax(df.get("SG_Long", df.get("SG_Baseline", 0)))
...
def compute_projections(df):
    df = df.copy()
    # placeholder mean using performance cluster + price; swap with your model later
...
def compute_projected_ownership(df, w: Weights):
    ...
    tier_prior = df.get("Salary_Tier", "Mid").map({"Stud":1.08,"Upper":1.05,"Mid":1.00,"Value":0.95})
    if isinstance(tier_prior, pd.Series): raw *= tier_prior.fillna(1.0)
    total = raw.sum()
...
def compute_gto_ownership(df, w: Weights):
    ...
    Q = Q.clip(lower=1e-6)
    total = Q.sum()
    slots_total = 600.0  # 150 lineups × 6 slots
...
def compute_diagnostics(df, po, gto):
    ...
    df["Tier_Floor_%"] = 0.5
    df["Cap_Flag"] = False
    df["TargetSlots"]   = np.rint(df["GTO_%"] * 1.5).astype(int)  # 150 lineups
...
def build_scorecard(raw_df: pd.DataFrame, weights: Weights) -> pd.DataFrame:
    ...
    preferred = [
        ...
        "PO_%","GTO_%","HypeGap_Z","Leverage_%","Tier_Floor_%","Cap_Flag","TargetSlots","MaxExposure_%","MinInclude"
    ]
    cols = [c for c in preferred if c in df.columns] + [c for c in df.columns if c not in preferred]
    return df[cols]

def preflight_feasibility(df, L=150, R=6, cap_pct=26.5, gto_floor=0.0):
...
if run_etl:
    ...
    if merged["Value_PPD_Baseline"].isna().all() and "RG_fpts" in merged:
